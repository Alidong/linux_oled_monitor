...
#endif
    //lable style
    style_init_reset(&styles->lable);
    lv_style_set_bg_opa(&styles->lable, LV_OPA_COVER);
    //lv_style_set_bg_color(&style, lv_palette_lighten(LV_PALETTE_GREY, 2));
    lv_style_set_bg_color(&styles->lable, lv_color_black());
    //lv_style_set_border_width(&style, 2);
    //lv_style_set_border_color(&style, lv_palette_main(LV_PALETTE_BLUE));
    lv_style_set_text_color(&styles->lable, lv_color_white());
    lv_style_set_text_letter_space(&styles->lable, 5);
    lv_style_set_text_line_space(&styles->lable, 20);
    lv_style_set_text_decor(&styles->lable, LV_TEXT_DECOR_UNDERLINE);
}
...
static void theme_apply(lv_theme_t *th, lv_obj_t *obj)
{
    ...
    {
        printf("lable!\r\n");
        lv_obj_add_style(obj, &styles->lable, LV_PART_MAIN | LV_STATE_DEFAULT);
    }
}
